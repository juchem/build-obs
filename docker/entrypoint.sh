...
build_obs() {
  ...
  build_version="${OBS_STUDIO_VERSION:-HEAD}"
  ...
  cef_download="/tmp/cef_binary_${OBS_CEF_VERSION}_linux64.tar.bz2"
  curl -sfLo "${cef_download}" \
    "https://cdn-fastly.obsproject.com/downloads/cef_binary_${OBS_CEF_VERSION}_linux64.tar.bz2"
...
build_obs_shaderfilter() {
  ...
  build_version="${OBS_SHADERFILTER_VERSION:-HEAD}"
  git fetch --depth=1 origin "${build_version}"
  git checkout -b "build-${build_version}-$(date +%s)" FETCH_HEAD
  git clean -xfd
